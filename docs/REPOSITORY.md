...
| `Generate_Tangerine_3D/procedural_track/configs/variants_batch.yaml` | 병해 변종 GLB 배치·베이크 해상도 등 (`DISEASE_MATERIALS.md`와 짝). |
| `Generate_Tangerine_3D/procedural_track/configs/base_mesh.yaml`      | (선택) 아이코스피어 베이스 빌드.                                   |
| `data/Tangerine_3D/glb_procedural/` / `glb_from_2d/`                 | 최종 GLB만 (트랙 1 / 트랙 2).                                      |
...
| `healthy_variants_export.py`, `fruit_class_mesh_export.py`, `decal_mesh_export.py`, `export_base_mesh.py`              | 형태·클래스별 GLB 등 보조 빌드 (트랙 2 UV 텍스처 스탬프는 `decal_mesh_export`).          |
| `metadata_schema.json`                                                                                                 | `frame_metadata.jsonl` 필드 설명.                                                  |


### `src/decal_prep/`

트랙 2 **데칼 전처리** (병변 마스크·RGBA 패치·manifest). `build_glb_from_2d.py` 가 CPython 단계에서 호출한다.
...
| `generate_variants_build.py`                                                   | 병해 변종 GLB 일괄 (`Generate_Tangerine_3D/procedural_track/configs/variants_batch.yaml`). |
| `build_glb_from_2d.py`, `seraph_build_glb_from_2d_remote.py`                     | 트랙 2: 병변 패치·UV 알베도 스탬프 GLB (기본) 또는 레거시 BOX 투영 — `glb_from_2d/`; Seraph SSH. |
| `healthy_variants_build.py`, `build_base_mesh.py`, `fruit_class_mesh_build.py` | 형태·베이스·클래스별 GLB (`fruit_class_mesh_build` 는 `build_glb_from_2d` 와 동일 Blender 로직, 출력 경로만 다름). |
