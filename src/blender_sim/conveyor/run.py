...
from src.blender_sim.conveyor import defaults
from src.blender_sim.conveyor import physics as phys
from src.blender_sim.conveyor.cycles_gpu import configure_cycles_device
from src.blender_sim.conveyor.camera import (
    setup_diagonal_camera,
    setup_factory_lights,
    setup_line_inspection_camera,
    setup_top_down_camera,
    setup_world_background,
)
from src.blender_sim.conveyor.conveyor_mesh import build_conveyor
from src.blender_sim.conveyor.conveyor_pitch import apply_conveyor_pitch
from src.blender_sim.conveyor.objects.fruit import (
    build_citrus_glb_sequence,
    build_glb_template,
    create_fruit_object,
)
from src.blender_sim.conveyor.roller_motion import setup_rollers_from_config
from src.blender_sim.conveyor.spawn import compute_spawn_frames, spawn_location_for_frame
...
def run_demo(raw_cfg: Dict[str, Any]) -> None:
    ...
    if eng == "CYCLES" and hasattr(scene.render, "engine"):
        ...
        if "EEVEE" in eng:
            _valid = {e.identifier for e in scene.render.bl_rna.properties["engine"].enum_items}
    ...
    belt_min, belt_max, spawn_min, spawn_max = apply_conveyor_pitch(scene, build, cfg)
    ...
    bl = float(cfg["belt_length_m"])
    bw = float(cfg["belt_width_m"])
    lens = float(cfg.get("camera_lens_mm", 40.0))
    ...
    elif cam_style in ("line_inspection", "line", "qc_line"):
        setup_line_inspection_camera(
            scene,
            look_at=center,
            belt_length=bl,
            belt_width=bw,
            lens_mm=float(cfg.get("camera_line_inspection_lens_mm", 35.0)),
            cfg=cfg,
        )
    else:
        setup_top_down_camera(
    ...
    setup_factory_lights(scene, center, bl, bw, cfg)
    ...
    if blend_rel:
        ...
        print(f"[conveyor] Saved .blend (Blender에서 열어 재생·뷰포트 확인) -> {blend_abs}")

    print(f"[conveyor] Intermediate PNGs -> {renders_dir}")
    print(f"[conveyor] Spawns completed: {spawn_idx} / {n_spawn}")
    if write_meta:
        print(f"[conveyor] Meta -> {meta_path}")
