...
- 기본: `data/Tangerine_3D/glb_procedural/` 에서 GLB 30개(병해 재질 유지), 폴더가 없으면 `Generate_Tangerine_3D/procedural_track/mesh_bases/` 3종×10
  ...
  python Conveyor_Lab/scripts/run_conveyor_demo.py --frames 300 --cycles 14

  python Conveyor_Lab/scripts/run_conveyor_demo.py --roller-speed 5.5

  python Conveyor_Lab/scripts/run_conveyor_demo.py --preview-blend --no-video   # .blend 만

  python Conveyor_Lab/scripts/run_conveyor_demo.py --also-blend                 # MP4 + conveyor_scene.blend

  python Conveyor_Lab/scripts/run_conveyor_demo.py --pitch-deg 12               # 기울기(도) 덮어쓰기
"""
...
ENTRY = ROOT / "src" / "blender_sim" / "entries" / "conveyor_entry.py"
_DEFAULT_OUT = ROOT / "Conveyor_Lab" / "outputs" / "conveyor_demo"
...
def main() -> int:
    ...
    p.add_argument(
        ...
        help="Output directory (default: Conveyor_Lab/outputs/conveyor_demo)",
    )
    ...
    p.add_argument(
        ...
        metavar="DEG",
        dest="pitch_deg",
        help="Conveyor tilt around world Y (+X downhill); maps to conveyor_pitch_deg",
    )
    ...
    if args.pitch_deg is not None:
        overrides["conveyor_pitch_deg"] = args.pitch_deg
    ...
    if not args.no_video:
        ...
        if ff:
            ...
            try:
                ...
                encoded_ok = mp4_out.is_file()
            except (subprocess.CalledProcessError, OSError):
                encoded_ok = False
            if encoded_ok:
                print(f"Video: {mp4_out}")
        if not encoded_ok and _stitch_pngs_to_mp4_opencv(renders_dir, mp4_out, fps):
            encoded_ok = True
            print(f"ffmpeg missing; wrote MP4 via OpenCV: {mp4_out}")
        if not encoded_ok:
            print(
                "MP4 encoding failed (install ffmpeg or: pip install imageio-ffmpeg). PNGs kept in",
                renders_dir,
            )
        ...
        if cfg.get("delete_intermediate_frames", True) and encoded_ok and renders_dir.is_dir():
